...
template <typename Dtype>
DiagonalAffineMap<Dtype> FilterMap(const int num_spatial_axes, const int* kernel_shape_data,
                                   const int* stride_data, const int* pad_data) {
  vector<pair<Dtype, Dtype> > coefs;
  for(int i = 0; i < num_spatial_axes; i++) {
    coefs.push_back(make_pair(stride_data[i],
                              static_cast<Dtype>(kernel_shape_data[i] - 1) / 2 - pad_data[i]));
  }
  return DiagonalAffineMap<Dtype>(coefs);
}
